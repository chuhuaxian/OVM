...
#ifndef _OVM_STATUS_H_
#define _OVM_STATUS_H_
...
namespace OVM
{
	...
	namespace Attributes
	{
		...
		*   Add status information to a base class. \c StatusBits
		*/
		class StatusInfo
		{
		public:
			...
			bool deleted() const  
			{ 
				return is_bit_set(DELETED); 
			}
			/** set deleted */
			void set_deleted(bool _b) 
			{
				change_bit(DELETED, _b); 
			}

			/** is locked ? */
			bool locked() const  
			{
				return is_bit_set(LOCKED); 
			}
			/** set locked */
			void set_locked(bool _b) 
			{
				change_bit(LOCKED, _b); 
			}

			/** is selected ? */
			bool selected() const  
			{ 
				return is_bit_set(SELECTED); 
			}
			/** set selected */
			void set_selected(bool _b) 
			{ 
				change_bit(SELECTED, _b); 
			}

			/** is hidden ? */
			bool hidden() const  
			{ 
				return is_bit_set(HIDDEN); 
			}
			/** set hidden */
			void set_hidden(bool _b) 
			{ 
				change_bit(HIDDEN, _b); 
			}

			/** is feature ? */
			bool feature() const 
			{
				return is_bit_set(FEATURE);
			}
			/** set feature */
			void set_feature(bool _b) 
			{ 
				change_bit(FEATURE, _b); 
			}

			/** is tagged ? */
			bool tagged() const 
			{
				return is_bit_set(TAGGED);
			}
			/** set tagged */
			void set_tagged(bool _b)
			{
				change_bit(TAGGED, _b);
			}

			/** is tagged2 ? This is just one more tag info. */
			bool tagged2() const  
			{
				return is_bit_set(TAGGED2); 
			}
			/** set tagged */
			void set_tagged2(bool _b) 
			{
				change_bit(TAGGED2, _b);
			}

			/** is fixed non-manifold ? */
			bool fixed_nonmanifold() const  
			{
				return is_bit_set(FIXEDNONMANIFOLD); 
			}
			/** set fixed non-manifold */
			void set_fixed_nonmanifold(bool _b) 
			{
				change_bit(FIXEDNONMANIFOLD, _b); 
			}
